...
class SEDChannel:
    @staticmethod
    def calculate_channel(n_ant: int, n_user: int, phase: Phase, frame_ind: int, fading: bool) -> np.ndarray:
        ...
        if fading and phase == Phase.TEST:
            H = SEDChannel.add_fading(H, phase, n_ant, frame_ind)

        return H

    @staticmethod
    def add_fading(H: np.ndarray, phase: Phase, n_ant: int, frame_ind: int) -> np.ndarray:
